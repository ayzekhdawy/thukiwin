...
export const TIPS: readonly Tip[] = [
  ...
  'Highlight text in any app before summoning ThukiWin to include it as context',
  '/think makes ThukiWin reason step by step before answering, great for hard questions',
  '/search pulls live web results into the chat so answers stay current',
  'Ctrl+W or Esc hides the window; ThukiWin keeps running in the background',
  ...
  'Keep Warm holds your active model ready in VRAM so there is no loading delay when you summon ThukiWin',
  ...
  'Keep Warm in Settings keeps your active model loaded between sessions so ThukiWin is always ready at full speed',
  ...
  'Crank the Context Window slider in Settings up if ThukiWin is forgetting earlier parts of long conversations',
  ...
  'Run `ollama ps` in a terminal to see which model is loaded, its size, and the active context length',
  {
    ...
    url: 'https://github.com/ayzekhdawy/thukiwin/blob/main/docs/tuning-context-window.md',
  },
];
